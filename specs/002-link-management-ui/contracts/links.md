...
- `200 OK` — page of the caller's links (`code`, `slug`, `destination_url`, `is_active`,
...
|---|---|---|
| Shown | Caller owns `code` | `200 OK` — the link's `code`, `slug`, `destination_url`, `is_active`, `expires_at`. |
...
**Input**: `url` (required), `slug` (optional — SEO decoration, not a lookup key), `expiresAt`
(optional).
...
| Outcome | When | Response |
|---|---|---|
| Created | `url` valid and safe; `slug` (if given) well-formed; `expiresAt` (if given) not in the past | New link row (system-generated `code` + optional `slug`) + Redis write-through (FR-013, FR-014); redirect to the link's detail page. |
...
| Rejected — malformed slug | `slug` given but not lowercase alphanumeric+hyphens, 3–32 chars | `400 Bad Request`, form re-rendered with a validation error (FR-007, FR-019); no link created. |
...
`slug` never affects `code` — the code is immutable once created.
...
| Outcome | When | Response |
|---|---|---|
| Updated | Caller owns `code`; any new `destinationUrl` passes the same validation as create; any new `slug` is well-formed; any new `expiresAt` is not in the past | Row updated + Redis write-through with the full new state (FR-013, FR-014). |
...
| Rejected — malformed slug | New `slug` given but not lowercase alphanumeric+hyphens, 3–32 chars | `400 Bad Request`, form re-rendered with a validation error (FR-007, FR-019); existing link left unchanged. |
