...
| `404 Not Found` | Code doesn't exist in Redis or Postgres, exists but is expired, or an `slug` segment was given that doesn't match the code's registered slug (FR-021) | `text/html; charset=utf-8` — a small, static, branded page (`redirect/internal/handler/assets/error-notfound.html`, constitution Principle II's fourth narrow exception) | Spec FR-005, FR-006, FR-021. No distinction is exposed between "never existed," "expired," or "slug mismatch" — the body is the exact same static asset for all three, so no wording ever reveals which one applies. |
| `410 Gone` | Code exists and is deactivated (regardless of expiration) — only reachable when no `slug` segment was given, or it matched | `text/html; charset=utf-8` — a small, static, branded page (`redirect/internal/handler/assets/error-gone.html`) | Spec FR-007, FR-008. Deactivation takes precedence over expiration. A slug mismatch is checked first and always yields 404 with the 404 page's body, never 410 (data-model.md), so this status/body never reveals a deactivated link's existence to a wrong-slug request. |
